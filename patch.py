...
def find_mean(filename,sheet_name,csv):
    colname1 = "TPS"
    colname2 = "TPS"
    df = pd.read_excel(filename,sheet_name)
    col_mean = df[colname1].mean()
...
def assign():
    ...
    for i, j in zip(sheetname1, csvname):
        ...
        find_mean(filename,sheet_name,csv)
        draw(filename,sheet_name,csv)
def draw(filename,sheet_name,csv):
    #draw line graph based on average and current stats
    sheets=["STKAvgTPS","successRate"]
    colname1 = "TPS"
    colname2 = "TPS"
    df = pd.read_excel(filename,sheet_name)
    col_mean = df[colname1].mean()
    for a in sheets:
        ...
        #print(df6)
        ax = plt.gca()
        df6.plot(kind = 'line', x= '_time', y= sheet, ax=ax)
